...
pub fn parse(frame: Frame) -> Result<Command, ParseError> {
    ...
    let command_name =
        str::from_utf8(parts[0].as_ref()).map_err(|_| ParseError::InvalidCommandName)?;
...
mod tests {
    ...
    fn rejects_unknown_command() {
        let frame = Frame::Array(vec![Frame::Bulk(Bytes::from_static(b"NOPE"))]);
    ...
    fn rejects_del_without_keys() {
        let frame = Frame::Array(vec![Frame::Bulk(Bytes::from_static(b"DEL"))]);
    ...
    fn rejects_exists_without_keys() {
        let frame = Frame::Array(vec![Frame::Bulk(Bytes::from_static(b"EXISTS"))]);
