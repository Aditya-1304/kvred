...
async fn ping_over_tcp() {
    let (addr, listener_handle, writer_handles, shutdown_tx, aof_path) = spawn_server("ping").await;
    ...
    stop_server(listener_handle, writer_handles, shutdown_tx).await;
...
async fn del_over_tcp_is_appended_to_aof() {
    ...
    let del_reply = send_and_read(&mut stream, b"*2\r\n$3\r\nDEL\r\n$5\r\nmykey\r\n").await;
